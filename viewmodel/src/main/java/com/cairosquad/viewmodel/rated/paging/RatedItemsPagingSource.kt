...
import com.cairosquad.viewmodel.rated.toUiState
...
class RatedItemsPagingSource(
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, MyRatingsScreenState.RatedItemUiState> {
        ...
        return try {
            ...
            val movieItems = movies.map { (movie, rating) -> movie.toUiState(rating) }
            val seriesItems = series.map { (series,rating) -> series.toUiState(rating) }
