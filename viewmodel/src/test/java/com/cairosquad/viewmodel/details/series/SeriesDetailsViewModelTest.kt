...
class SeriesDetailsViewModelTest {
    ...
    fun `init SHOULD load all data sections`() = runTest {
        ...
        coEvery { mockUseCase.getSeriesTopCast(seriesId, page = 1) } returns emptyList()
        coEvery { mockUseCase.getSeriesSeasons(seriesId) } returns emptyList()
        coEvery { mockUseCase.getSeriesReviews(seriesId, page = 1) } returns emptyList()
        coEvery { mockUseCase.getSimilarSeries(seriesId, page = 1) } returns emptyList()
        ...
        coVerify(exactly = 1) {
            ...
            mockUseCase.getSeriesTopCast(seriesId, page = 1)
            mockUseCase.getSeriesSeasons(seriesId)
            mockUseCase.getSeriesReviews(seriesId, page = 1)
            mockUseCase.getSimilarSeries(seriesId, page = 1)
        }
    }
